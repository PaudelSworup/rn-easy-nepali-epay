...
export class Base64 {
  ...
  public static stringify(wordArray: WordArray): string {
    ...
    for (let i = 0; i < wordArray.sigBytes; i += 3) {
      const byte1 =
        (wordArray.words[i >>> 2] ?? 0 >>> (24 - (i % 4) * 8)) & 0xff;
      const byte2 =
        (wordArray.words[(i + 1) >>> 2] ?? 0 >>> (24 - ((i + 1) % 4) * 8)) &
        0xff;
      const byte3 =
        (wordArray.words[(i + 2) >>> 2] ?? 0 >>> (24 - ((i + 2) % 4) * 8)) &
        0xff;
  ...
  public static parseLoop(
    ...
    for (let i = 0; i < base64StrLength; i++) {
      if (i % 4) {
        const bits1 =
          reverseMap[base64Str.charCodeAt(i - 1)] ?? 0 << ((i % 4) * 2);
        const bits2 =
          reverseMap[base64Str.charCodeAt(i)] ?? 0 >>> (6 - (i % 4) * 2);
